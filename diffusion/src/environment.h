...
class microenvironment {
    public:
        ...
        uint  x_size, y_size, z_size , number_of_densities; //Size of each dimension
        double cube_side, delta; 
        uint i_jump, j_jump, k_jump;
        ...
        void initialize_diffusion(double cube_side, double delta, int d_num, int rank, int size, int vl, MPI_Comm mpi_world);

        void set_dirichlets();

        //Trifdiagonal functions
        void apply_dirichlet_conditions(); //
        void apply_dirichlet_conditions_boundaries();

        void diffusion_decay(); 
        void diffusion_decay_512();

        void diffusion_decay_novector(); 
