...
void microenvironment::initialize_diffusion(double cube_side, double delta, int d_num, int rank, int size, int vl, MPI_Comm mpi_world){
    ...
    //Simulation
    long long vec_size = x_size;
    vec_size *= y_size;
    vec_size *= z_size;
    vec_size *= number_of_densities;
    long long sub = densities.max_size() - vec_size;
    densities.resize(vec_size, 0.0);
    if (densities.size() != vec_size) {
        cout << "Rank " << mpi_rank << " has not allocated the microenvironment correctly!" << endl;
        cout << "   Allocated " << densities.size() << " expected " << x_size*y_size*z_size*number_of_densities << endl;
        abort();
    }
